...
% 	I was also not able to find the definition of tempered distributions that Prof.\@ Schuller provides in any standard references. All of the references I checked define a \textbf{tempered distribution as a continuous linear functional on the Schwartz space}, i.e. $\distr$ as simply the topological dual of $\schwartzr$. Prof.\@ Schuller defines $\schwartzr$ as a subset of the algebraic dual $\distar$, characterised by the specific form of $\Phi_m [f]$. He does not put any topology explicitly on $\schwartz$, but he defines a seminorm on $\schwartz$. The topology on $\schwartzr$ can be assumed to be the topology induced by these seminorms. It might be useful to look at Theorem V.10 and p.~145 of the first volume of book of Reed and Simon~\cite{Reed}, to this effect.
	...
	We denote the space of all continuous anti-linear functionals on $\schwartzr$ by $\schwartz^\dagger(\rr)$. If $\Phi \in \distr$, then we define the vector space isomorphism
	\begin{align*}
	    \Lambda &\colon \distr \rightarrow \schwartz^\dagger(\rr),\\
		\Lambda &\colon \Phi \rightarrow \Lambda(\Phi),
	\end{align*}
	by
	\begin{align*}
	...
	Just like we embedded $\ltwor$ in $\schwartzr$, we can embed $\ltwor$ in $\schwartz^\dagger(\rr)$. We define the embedding
	\begin{align*}
		\omega &\colon \ltwor \rightarrow \schwartz^\dagger(\rr),\\
	...
	It can be shown that the above embedding function defines each $\Psi_\phi[f]$ to be a continuous linear functional for all $\phi \in \ltwor$ and $f \in \schwartzr$. Just like the case for tempered distributions, we thus have $\ltwor \xhookrightarrow{} \schwartz^\dagger(\rr)$, and by abuse of notation, $\omega(\ltwor) \subset \distr$ can be written as $\ltwor \subset \schwartz^\dagger(\rr)$.

	Elements of $\schwartz^\dagger(\rr)$ are identified with \textit{kets} and elements of $\distr$ are identified with \textit{bras} in quantum mechanics. Thus, we can call $\schwartz^\dagger(\rr)$ the \textit{ket-space} and $\distr$ the \textit{bra-space}. Since $\Lambda$ is an isomorphism, there is a one-to-one correspondence between kets and bras. It should be noted that some authors such as Bowers~\cite[chp.~5]{Bowers} call the images of the embeddings $\ltwor \xhookrightarrow{} \distr$ and $\ltwor \xhookrightarrow{} \schwartz^\dagger(\rr)$ as the bra-space and the ket-space respectively. We recommend the reader look up chp.~5 in the book of Bowers~\cite{Bowers} for a discussion of the Dirac calculus in a mathematically rigorous fashion.
