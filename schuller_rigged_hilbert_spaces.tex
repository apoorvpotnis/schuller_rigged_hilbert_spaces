...
% \the\everymath%
% \Umathsubshiftdown\textstyle=1.75pt\Umathsubshiftdrop\textstyle=0.5pt}
% \everydisplay=\expandafter{%
% \the\everydisplay%
% \Umathsubshiftdown\displaystyle=1.75pt\Umathsubshiftdrop\displaystyle=0.5pt}
...
\begin{filecontents}[overwrite]{schuller_rigged_hilbert_spaces.bib}
    ...
    @misc{riggedphyse,
        author = {User `user1504' (\url{https://physics.stackexchange.com/users/1504/user1504})},
    ...
    @misc{riggedmo,
        author = {User `MathMath' (\url{https://mathoverflow.net/users/152094/mathmath})},
    ...
    @misc{zwarich,
        author = {Cameron Zwarich (\url{https://mathoverflow.net/users/99234/cameron-zwarich})},
        howpublished = {MathOverflow (\url{https://mathoverflow.net})},
        note = {Version: 2017-11-08 18:50:20Z},
        title = {Comment on an answer to the question `Spectral theorem for self-adjoint differential operator on Hilbert space'},
        url = {https://mathoverflow.net/questions/125655/spectral-theorem-for-self-adjoint-differential-operator-on-hilbert-space/126758#comment705509_126758}
    }
    ...
    @misc{riggedmurray,
        author = {User `J.\@ Murray' (\url{https://physics.stackexchange.com/users/156895/j-murray})},
        howpublished = {Physics Stack Exchange (\url{https://physics.stackexchange.com})},
    ...
    @book{Blanchard,
        ...
        subtitle = {Distributions, Hilbert Space Operators, Variational Methods, and Applications in Quantum Physics},
        year = {2015},
        volume = 69,
    ...
    @mvbook{gelfand,
        ...
        subtitle = {Applications of Harmonic Analysis},
        publisher = {Academic Press, Inc. London},
        year = {1964},
        isbn = {0-12-279504-0},
        origlanguage = {russian},
        volume = 4,
        addendum = {The statements as cited appear on p.~126. However, it has been pointed out by Cameron Zwarich~\cite{zwarich} that the actual proof of the generalized eigenvector expansion given here is completely wrong. Gould's paper~\cite{gould} contains the correct proof.}
    }
    ...
    @book{bohm,
        author = {Bohm, Arno and Gadella, Manuel},
        isbn = {3-540-51916-5},
        title = {Dirac Kets, Gamow Vectors and Gel'fand Triplets},
        subtitle = {The Rigged Hilbert Space Formulation of Quantum Mechanics},
        year = {1969},
...
\newcommand{\identity}{\symbb{I}}
\DeclareMathOperator{\spec}{spec}
\newcommand{\resolvent}{\rho}

\DeclarePairedDelimiter{\norm}{\lVert}{\rVert}
\DeclarePairedDelimiter{\abs}{\lvert}{\rvert}
% \newcommand{\der}{\operatorname{d\!}{}}
\newcommand{\der}{\mathop{}\!\symrm{d}}
...
\begin{document}
    ...
    It was seen in the previous lectures that a self-adjoint operator acting on an infinite-dimensional Hilbert space need not admit a basis of eigenvectors in the space. For example, consider the position operator $\position$ on the Hilbert space \(\ltwor\), defined on a dense domain \(\domain(\position) \subset \ltwor\) as follows. The domain $\domain(\position)$ is determined by Stone's  theorem.
    \begin{align*}
        \position &\colon \domain(\position) \rightarrow \ltwor,\\
    ...
    Let $\hilbert$ be a Hilbert space over $\cc$. A $\lambda \in \rr$ is said to be an eigenvalue of a linear operator $A \colon \domain(A) \rightarrow \hilbert$ with the eigenvector $\psi \in \domain(A)\setminus\{0\}$ if the eigenvalue equation
    \[A\psi = \lambda \psi\]
    is satisfied. Here, \(\domain(A) \subset \ltwor\).
    ...
    While we construct these spaces for $\ltwor$ here, they can be constructed for more abstract Hilbert spaces as well. We thus `rig' the Hilbert space $\symcal{H}$ with two other spaces $\symcal{S}$ and $\symcal{S}^\times$:
    \[
    \symcal{S} \subset \hilbert \subset \symcal{S}^\times.
    \]
    The triple $(\symcal{S}, \hilbert, \symcal{S}^\times)$ is called a \textit{Gelfand triple}, in honour of the Russian mathematician Israel Moiseevich Gelfand.
    ...
    \end{align*}
    $f$ and all its derivatives vanish at $+\infty$ since $f \in \schwartzr$. Thus, we are left with
    \begin{align*}
        \dirac_\lambda [f] &= \lambda \cdot f^{(1)}(\lambda) - f(\lambda) - \lambda \cdot f^{(1)}(\lambda),\\
        \dirac_\lambda [f] &= f(\lambda).
    \end{align*}

    We now look at plane waves $E_k \in \distr$ of wavenumber $k \in \rr$. These are very important for quantum mechanics. $E_k$ is defined by \[(E_k)_0 (x) = \e^{\i kx}\] and $(E_k)_m = 0$ for all $m \neq 0$. Note that $E_k \notin \ltwor \supset \schwartzr$.

    \section{Embedding of \texorpdfstring{\(\ltwor\)}{L²(ℝ)} in \texorpdfstring{\(\distr\)}{S⁺(ℝ)}}
    ...
    \section{Lifting differential operators from \texorpdfstring{\(\schwartzr\)}{S(ℝ)}}
    ...
    \]
    Here, $M$ is a finite subset of $\nn$, and $D_m$ is a complex polynomial, i.e.\@ $D_m \in \cc[x]$. Such a $D$ is called a differential operator. By the defining properties of the Schwartz space, it can be seen that the operator is well-defined---the range is $\schwartzr$. $D$ has this nice property of mapping into the space itself.
    ...
    \]
    where $f \in \schwartzr$ and $M$ is a finite subset of $\nn$.\footnote{The lift formula as provided in the lecture contains a slight mistake. The correct formula provided here has been taken from eq.~(V\kern-1.5pt.5) of~\cite[p.~149]{Reed}.} Weak derivatives are defined using the integration by parts formula and the $(-1)^m$ factor is a consequence of this.

    \section{Spectrum of self-adjoint operators}

    Let $A \colon \domain(A) \rightarrow \hilbert$ be a self-adjoint operator, where $\domain(A)$ is a dense subset of $\hilbert$. We denote the range of the operator $(A - \lambda\identity)$ by $\operatorname{ran}(A - \lambda\identity)$, where $\lambda \in \cc$ and $\identity \colon \hilbert \rightarrow \hilbert$ denotes the identity function on $\hilbert$. We say that $\lambda \in \cc$ is a \textit{regular point} if and only if
    \[
    \operatorname{ran}(A - \lambda\identity) = \overline{\operatorname{ran}(A - \lambda\identity)} = \hilbert.
    \]
    Here, $\overline{\operatorname{ran}(A - \lambda\identity)}$ denotes the topological closure of $\operatorname{ran}(A - \lambda\identity)$. The set of regular points is called a \textit{resolvent set}. The complement of the resolvent set is known as the \textit{spectrum} of the operator, denoted by $\spec(A)$. If we denote the resolvent set of $A$ by $\resolvent(A)$, we have that \[\spec(A) = \cc \setminus \resolvent(A).\]
    ...
    \begin{enumerate}
        ...
        \[
        \spec_\text{pp}(A) \coloneq \{\lambda \in \cc \mid \operatorname{ran}(A - \lambda\identity) = \overline{\operatorname{ran}(A - \lambda\identity)} \neq \hilbert\}.
        \]
        \item The \textit{point embedded in continuum spectrum} of $A$, $\spec_{\text{pec}}(A)$, is defined as
        \[
        \spec_{\text{pec}}(A) \coloneq \{\lambda \in \cc \mid \operatorname{ran}(A - \lambda\identity) \neq \overline{\operatorname{ran}(A - \lambda\identity)} \neq \hilbert\}.
        \]
        \item The \textit{purely continuous spectrum} of $A$, $\spec_\text{pc}(A)$, is defined as
        \[
        \spec_\text{pc}(A) \coloneq \{\lambda \in \cc \mid \operatorname{ran}(A - \lambda\identity) \neq \overline{\operatorname{ran}(A - \lambda\identity)} = \hilbert\}.
        \]
    \end{enumerate}
    ...
    \begin{align*}
        ...
        \spec_{\symrm{p}}(A) &= \{\lambda \in \cc \mid\overline{\operatorname{ran}(A - \lambda\identity)} \neq \hilbert\}.
    \end{align*}
    We similarly define the \textit{continuous spectrum} of $A$, $\spec_\text{c} (A)$, to be the union of the purely continuous and the point embedded in continuum spectra.
    \begin{align*}
        \spec_{\symrm{c}}(A) &=  \spec_\text{pc}(A) \cup \spec_{\text{pec}}(A)\\
        \spec_{\symrm{c}}(A) &= \{\lambda \in \cc \mid \operatorname{ran}(A - \lambda\identity) \neq \overline{\operatorname{ran}(A - \lambda\identity)}\}.
    \end{align*}
    ...
    \]
    is known as a \textit{generalised eigenvalue} equation with the \textit{generalised eigenvector} or \textit{eigendistribution} $\Phi \in \distr \setminus \{0\}$ and $\lambda \in \cc$. The set of generalised eigenvalues $\lambda$ satisfying the above equation is defined as the spectrum of $A$.
    ...
    Just like we embedded $\ltwor$ in $\distr$, we can embed $\ltwor$ in $\anti$. We define the embedding
    \begin{align*}
    ...
    Elements of $\anti$ are identified with \textit{kets}, and elements of $\distr$ are identified with \textit{bras}, in quantum mechanics. Thus, we can call $\anti$ the \textit{ket-space} and $\distr$ the \textit{bra-space}~\cite[p.~3]{Ticciati}. Since $\Lambda$ is an isomorphism, there is a one-to-one correspondence between kets and bras. It should be noted that some authors such as Bowers~\cite[chp.~5]{Bowers} call the images $\eta(\ltwor)$ and $\omega(\ltwor)$ of the embeddings $\ltwor \xhookrightarrow{} \distr$ and $\ltwor \xhookrightarrow{} \anti$ as the bra-space and the ket-space respectively. We recommend that the reader consult chp.~5 in the book of Bowers~\cite{Bowers} for a discussion of the Dirac calculus in a mathematically rigorous fashion.
